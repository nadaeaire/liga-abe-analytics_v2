...
def render_view(id_jugador, df_games, df_players, df_rosters, df_teams):
    ...
    with c_t1:
        ...
        if st.button("📝 Game Log", key="btn_tab_log", use_container_width=True, type=type_btn1):
            guardar_actividad("Navegación", "Perfil Jugador - Click Pestaña Game Log") # <--- TRACKING
            st.session_state.active_tab_player = "Game Log"
            st.rerun()
    with c_t2:
        type_btn2 = "primary" if st.session_state.active_tab_player == "Trends" else "secondary"
        if st.button("📈 Trends tiro y eficiencias", key="btn_tab_trends", use_container_width=True, type=type_btn2):
            guardar_actividad("Navegación", "Perfil Jugador - Click Pestaña Trends") # <--- TRACKING
